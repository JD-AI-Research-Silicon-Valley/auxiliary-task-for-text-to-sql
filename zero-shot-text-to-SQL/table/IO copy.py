...
class TableDataset(torchtext.data.Dataset):
    ...
    def __init__(self, anno, fields, opt, filter_ex, **kwargs):
        ...
        BIO_label_data = self._read_annotated_file(
            opt, js_list, 'BIO_label', filter_ex)
        BIO_label_examples = self._construct_examples(BIO_label_data, 'BIO_label')
        BIO_label_loss_examples = self._construct_examples(BIO_label_data, 'BIO_label')

        #column_label_data = self._read_annotated_file(
        #    opt, js_list, 'column_label', filter_ex)
        #column_label_examples = self._construct_examples(column_label_data, 'src')


        #self.creat_label(opt, src_data, span_data)
        #self.creat_label(opt,src_data,span_data)
        ...
        examples = [join_dicts(*it) for it in zip(src_examples, ent_examples, agg_examples, sel_examples, lay_examples, tbl_examples, tbl_split_examples, tbl_mask_examples,
                                                  ...
                                                  BIO_label_examples, BIO_label_loss_examples)]
    ...
    def _read_annotated_file(self, opt, js_list, field, filter_ex):
        ...
        if field in ('sel', 'agg'):
        ...
        elif field in('BIO_label','column_label'):
            def _find_span(q_list, where_list):
                r_list = []
                for col, op, cond in where_list:
                    tk_list = cond['words']
            ...
            if field in ('BIO_label',):
                ...
                lines_3= ([col for col, op, cond in line['query']['conds']]
                     for line in js_list)
                lines = []
    ...
    def get_fields():
        ...
        fields["BIO_label"] = torchtext.data.Field(
            use_vocab=False, include_lengths=False, pad_token=PAD_WORD)
