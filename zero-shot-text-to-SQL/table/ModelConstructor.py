...
from table.Models import ParserModel, RNNEncoder, CondDecoder, TableRNNEncoder, MatchScorer, CondMatchScorer, CoAttention,CoAttention_wornn, AttnPool, Softattention
...
def make_encoder(opt, embeddings, ent_embedding=None, type_embedding=None):
    # "rnn" or "brnn"
    return RNNEncoder(opt.rnn_type, opt.brnn, opt.enc_layers, opt.rnn_size, opt.dropout, opt.lock_dropout, opt.weight_dropout, embeddings, ent_embedding,type_embedding)
...
def make_co_attention_wornn(opt):
    if opt.co_attention:
        return CoAttention_wornn(opt.rnn_type, opt.brnn, opt.enc_layers, opt.rnn_size, opt.dropout, opt.weight_dropout, opt.global_attention, opt.attn_hidden)
    return None


def make_self_attention(opt):
    return AttnPool(opt.rnn_size, opt.attn_hidden)
...
def make_base_model(model_opt, fields, checkpoint=None):
    ...
    #elmo_embeddings = make_elmo_embeddings(model_opt, fields['src'], )
    ...
    if model_opt.type_vec_size > 0:
        type_embedding = make_embeddings(
            fields["type"].vocab, model_opt.type_vec_size)
    else:
        type_embedding = None

    # Make question encoder.
    if model_opt.use_type =='use':
        q_encoder = make_encoder(model_opt, w_embeddings, ent_embedding, type_embedding)
    else:
    ...
    op_selfattention= make_co_attention_wornn(model_opt)

    agg_self_attention = make_self_attention(model_opt)
    lay_self_attention = make_self_attention(model_opt)

    BIO_classifier = nn.Sequential(
        nn.Dropout(model_opt.dropout),
        nn.Linear(model_opt.rnn_size, 4) #B,I,O and col
    )
    BIO_op_classifier = nn.Sequential(
        nn.Dropout(model_opt.dropout),
        nn.Linear(model_opt.rnn_size, 3)
    )
    ...
    decode_softattention=Softattention(model_opt.rnn_size,model_opt.attn_hidden)
    # embedding
    # layout encoding
    ...
    label_col_match = CondMatchScorer(
        MatchScorer(2 * model_opt.rnn_size, model_opt.score_size, model_opt.dropout,log_softmax=False))
    ...
    model = ParserModel(q_encoder, tbl_encoder, co_attention, agg_classifier, sel_match, lay_classifier, cond_embedding,
                        lay_encoder, cond_decoder, cond_col_match, cond_span_l_match, cond_span_r_match, model_opt, pad_word_index, agg_self_attention,lay_self_attention,decode_softattention,BIO_classifier,BIO_op_classifier,label_col_match,op_selfattention)
