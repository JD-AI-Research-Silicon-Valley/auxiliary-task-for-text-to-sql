...
class RNNEncoder(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional, num_layers,
                 hidden_size, dropout, lock_dropout, weight_dropout, embeddings, ent_embedding, type_embedding=None):
        ...
        if type_embedding is not None:
            input_size += type_embedding.embedding_dim
    ...
    def forward(self, input, lengths=None, hidden=None, ent=None, type=None):
        emb = self.embeddings(input)
        if self.ent_embedding is not None:
            emb_ent = self.ent_embedding(ent)
            emb = torch.cat((emb, emb_ent), 2)
        if self.type_embedding is not None:
            emb_type = self.type_embedding(type)
            emb = torch.cat((emb, emb_type), 2)
...
class MatchScorer(nn.Module):
    def __init__(self, input_size, score_size, dropout,log_softmax=True):
    ...
    def forward(self, q_enc, tbl_enc, tbl_mask, previous_distribution=None,select=False):
        """
        ...
        #print(q_enc_expand.size(),tbl_enc.size())
        #print((q_enc_expand*tbl_enc).size())
        if select:
            score=torch.sum(q_enc_expand*tbl_enc ,dim=2).transpose(0,1)
        else:
            score = self.score_layer(feat).squeeze(2)

        # mask scores
        score_mask = score.masked_fill(tbl_mask.type(torch.bool), -1e20)#-float('inf'))
...
class CondMatchScorer(nn.Module):
    ...
    def forward(self, cond_context_filter, tbl_enc, tbl_mask, previous_distribution=None,emb_span_l=None):
        """
        ...
        for cond_context_one in cond_context_filter:
            # -> (batch, num_table_header)
            r_list.append(self.sel_match(cond_context_one, tbl_enc, tbl_mask,previous_distribution=previous_distribution))
...
class CoAttention_wornn(nn.Module):
    def __init__(self, rnn_type, bidirectional, num_layers, hidden_size, dropout, weight_dropout, attn_type, attn_hidden):
        super(CoAttention_wornn, self).__init__()
    ...
    def forward(self, q_all, lengths, tbl_enc, tbl_mask):
        ...
        emb, _ = self.attn(
            q_all.transpose(0, 1).contiguous(),  # (output_len, batch, d)
            tbl_enc.transpose(0, 1).contiguous()     # (contxt_len, batch, d)
        )
...
class AttnPool(nn.Module):
    def __init__(self, hidden_size, attn_size):
        ...
        self.m=nn.Sequential(
            nn.Linear(self.hidden_size, self.attn_size, bias=True),
            nn.Tanh(),
            nn.Linear(self.attn_size, 1, bias=False),#len,bsz,1
        )

    def forward(self, q_all, lengths):
        #self.attn.applyMask(tbl_mask.data.unsqueeze(0))
        # attention
        ...
        q_att = self.m(q_all)
        q_att = torch.squeeze(q_att,dim=2)# len,bsz
        #print(q_all.size(),q_att.size())
        q_att=torch.transpose(q_att,0,1)
        #print(q_all.size(), q_att.size())
        q_att=torch.transpose(self.softmax(q_att),0,1)
        q_att=torch.unsqueeze(q_att,dim=2)
        #print(q_all.size(), q_att.size())
        #print(q_att)
        #print(q_att.size())
        q_all = torch.sum(q_att * q_all, dim=0)
...
class Softattention(nn.Module):
    def __init__(self,hidden_size, attn_size):
        ...
        self.attn_size = attn_size
        self.m=nn.Sequential(
            nn.Linear(self.hidden_size*2, self.attn_size, bias=True),
            nn.Tanh(),
            nn.Linear(self.attn_size, 1, bias=False),#len,bsz,1
        )
    ...
    def forward(self, hidden, q_all, lengths):
        # hidden (#cond,bsz, #hidden_sz) , q_all (len, bsz, #hidden_sz)
        #->hidden (#cond, len, bsz, #hidden_sz) , q_all (#cond, len, bsz, #hidden_sz)
        #print(hidden.size(), q_all.size())
        new_hidden=hidden.unsqueeze(1).repeat(1,q_all.size(0),1,1)
        new_q_all= q_all.unsqueeze(0).repeat(hidden.size(0),1,1,1)
        #print(new_hidden.size(),new_q_all.size())
        x=torch.cat( [new_hidden,  new_q_all], dim=-1)
#        print(x.size())
        x = self.m(x)
#        print(x.size())
...
#        print(x.size())
        #print mask.size()
        mask=sequence_mask(lengths).eq(0).type(torch.bool)   #len,bsz
        mask=mask.transpose(0,1)
        mask=mask.unsqueeze(0).repeat(hidden.size(0),1,1).unsqueeze(3) #cond,len,bsz
        #print(x.size(),mask.size())
        x.masked_fill_(mask,-1e10)

        x = F.softmax(x, dim=1)

        return self.MLP( torch.cat([hidden,torch.sum(x *new_q_all,dim=1)],dim=-1))


class tbl2q_attention(nn.Module):
    def __init__(self,hidden_size, attn_size):
        super(tbl2q_attention, self).__init__()
        self.hidden_size = hidden_size
        self.attn_size = attn_size
        self.m=nn.Sequential(
            nn.Linear(self.hidden_size*2, self.attn_size, bias=True),
...
#        print(x.size())
        ...
        #print(self.MLP( torch.cat([hidden,torch.sum(x *new_q_all,dim=1)],dim=-1)).size())
        return self.MLP( torch.cat([hidden,torch.sum(x *new_q_all,dim=1)],dim=-1))


class ParserModel(nn.Module):
    def __init__(self, q_encoder, tbl_encoder, co_attention, agg_classifier, sel_match, lay_classifier, cond_embedding, lay_encoder, cond_decoder,
                 cond_col_match, cond_span_l_match, cond_span_r_match, model_opt, pad_word_index, agg_self_attention,lay_self_attention,decode_softattention, BIO_classifier,BIO_op_classifier,label_col_match,op_selfattention):
        ...
        self.agg_self_attention =  agg_self_attention
        self.lay_self_attention=lay_self_attention
        self.decode_softattention=decode_softattention
        self.BIO_classifier=BIO_classifier
        self.BIO_op_classifier=BIO_op_classifier
        self.label_col_match=label_col_match
        self.op_selfattention=op_selfattention
        self.tbl2q_att=tbl2q_attention(model_opt.rnn_size,model_opt.attn_hidden)
        vocab_dic = dict(torch.load(os.path.join(self.opt.data, 'vocab.pt')))

        self.id2words = vocab_dic['src'].itos

        self.FC=nn.Sequential(
            nn.Dropout(model_opt.dropout),
            nn.Linear(model_opt.rnn_size*2, model_opt.rnn_size),
            )

    def enc(self, q, q_len, ent, type, tbl, tbl_len, tbl_split, tbl_mask):
        q_enc, q_all = self.q_encoder(q, lengths=q_len, ent=ent, type=type)
        tbl_enc = self.tbl_encoder(tbl, tbl_len, tbl_split)

        tbl_enc_new=self.tbl2q_att(tbl_enc,q_all,q_len)

        #print(tbl_mask.size(), tbl_enc.size())
        ...
        return q_enc, q_all, tbl_enc_new, q_ht, batch_size

    def select3(self, cond_context, start_index):
        return cond_context[start_index:cond_context.size(
            0):3]

    def forward(self, q, q_len, ent, type, tbl, tbl_len, tbl_split, tbl_mask, cond_op, cond_op_len, cond_col, cond_span_l, cond_span_r, lay):
        fff = 0
        if random.random() < 0.000:
            fff = 1
        # encoding
        q_enc, q_all, tbl_enc, q_ht, batch_size = self.enc(
            q, q_len, ent, type, tbl, tbl_len, tbl_split, tbl_mask)

        BIO_op_out = self.BIO_op_classifier(q_all)
        ...
        BIO_out = self.BIO_classifier(q_all)
        ...
        BIO_col_out = self.label_col_match(q_all, tbl_enc, tbl_mask)
        ...
        if fff == 1:
            print([self.id2words[w] for w in  q.transpose(0,1)[0].data ])
            print([self.id2words[w] for w in tbl.transpose(0, 1)[0].data ])
            print(BIO_out_sf.transpose(0, 1)[0])
            print(BIO_op_out_sf.transpose(0, 1)[0])
            print(BIO_col_out_sf.transpose(0, 1)[0])




        #q_all=self.FC(torch.cat([q_all,BIO_out],dim=2))
        # (1) decoding
        q_self_encode=self.agg_self_attention(q_all,q_len)
        agg_out = self.agg_classifier(q_self_encode) #select op
        #agg_out = self.agg_classifier(q_ht)
        sel_out = self.sel_match(q_self_encode, tbl_enc, tbl_mask,select=True)  # select column
        #sel_out = self.sel_match(q_ht, tbl_enc, tbl_mask)                   #select column
        q_self_encode_layout=self.lay_self_attention(q_all,q_len)
        lay_out = self.lay_classifier(q_self_encode_layout)                               #layout predict
        ...
        cond_context, _, _ = self.cond_decoder(emb, q_all, q_state)# input in decode, encode hidden, initial decode hidden


        # cond col
        cond_context_0 = self.select3(cond_context, 0)#pick one of each 3
        #print(cond_context_0.size(),q_self_encode.size())

        #print(cond_context_0.size())
#        cond_context_0=self.decode_softattention(cond_context_0,q_all,q_len)
        #print(cond_context_0.size())
#        print('\n')
        ...
        cond_span_r_out = self.cond_span_r_match(
            cond_context_1, q_all, q_mask, emb_span_l=emb_span_l)

        return agg_out, sel_out, lay_out, cond_col_out, cond_span_l_out, cond_span_r_out,BIO_out,BIO_col_out,BIO_op_out
