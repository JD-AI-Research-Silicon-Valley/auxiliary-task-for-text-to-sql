...
class GlobalAttention(nn.Module):
    ...
    * general: $$score(h_t,{\overline{h}}_s) = h_t^T W_a {\overline{h}}_s$$
    ...
    def applyMaskBySeqBatch(self, q):
        self.applyMask(q.data.eq(table.IO.PAD).t().contiguous().unsqueeze(0).type(torch.bool))
    ...
    def forward(self, input, context):
        ...
        if self.mask is not None:
            mask_ = self.mask.view(batch, 1, sourceL).type(torch.bool)  # make it broardcastable
